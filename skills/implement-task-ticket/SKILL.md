...
Where `<slug>` comes from the ticket filename (e.g., `ticket/004-index-builder`).
...
**Type:** `explore`
**Task:** Read the ticket at `planning/tasks/NNN-*.md`. Parse the `Related Tickets` section. For each related ticket number, read that ticket's YAML frontmatter and return its `status`. If ANY dependency has `status: "To Do"`, return a BLOCKED result listing which tickets must be completed first.

#### Subagent C — Read design doc
**Type:** `explore`
**Task:** Read `planning/design.md`. Identify the section(s) referenced in the ticket's `Design Doc Section` field (ticket is at `planning/tasks/NNN-*.md`). Return the full content of those relevant sections plus a brief summary of how they relate to the ticket.
...
3. **Otherwise** — you now have everything needed. Combine the subagent summaries into a mental model of:
...
2. Create or modify files as specified in `Technical Notes and Implementation Hints` and ask user for feedbacks.
...
Update the ticket file's frontmatter:

If all criteria are complete
```yaml
status: "Done"
```

If some criteria are incomplete
```yaml
status: "In Progress"
```
